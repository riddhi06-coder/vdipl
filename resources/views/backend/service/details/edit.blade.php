                        ...
                        <div class="row g-3">    
                            ...
                            <div class="tab-content" id="wizard-tabContent">
                                <div class="tab-pane fade show active" id="wizard-contact" role="tabpanel" aria-labelledby="wizard-contact-tab">
                                    <form class="row g-3 needs-validation custom-input" novalidate action="{{ route('manage-service-intro.update', $details->id) }}" method="POST" enctype="multipart/form-data">
                                        @csrf
                                        @method('PUT')
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <div id="ImagePreviewContainer" style="{{ $details->image ? 'margin-top: 10px;' : 'display: none; margin-top: 10px;' }}">
                                                @if($details->image)
                                                    <img id="image_preview" src="{{ asset('uploads/service/' . $details->image) }}" alt="Image" style="max-height: 200px;" class="mt-2">
                                                @endif
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-md-12" style="margin-bottom: 20px;">
                                            ...
                                            <textarea id="summernote" class="form-control" name="description" rows="5" placeholder="Enter Description here"  >{{ old('description', $details->description) }}</textarea>
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <input class="form-control" id="section_heading2" type="text" name="section_heading2" placeholder="Enter Section Heading" value="{{ old('section_heading2' , $details->section_heading2) }}" >
                                            <div class="invalid-feedback">Please enter a Section Heading.</div>
                                        </div>
                                                ...
                                                <tbody>
                                                    @php
                                                        ...
                                                        if ($oldEntries) {
                                                            ...
                                                            $titles = json_decode($details->banner_titles ?? '[]', true) ?? [];
                                                            $images = json_decode($details->banner_images ?? '[]', true) ?? [];
                                                            $descriptions = json_decode($details->banner_descriptions ?? '[]', true) ?? [];
                                                    ...
                                                    @foreach ($entries as $index => $entry)
                                                        <tr>
                                                            <td>
                                                                <input type="text" name="banner_items[{{ $index }}][name]" class="form-control"
                                                                    value="{{ $entry['name'] ?? '' }}" >
                                                            </td>
                                                            <td>
                                                                <input type="file" name="banner_items[{{ $index }}][image]" class="form-control image-input" {{ isset($entry['image']) ? '' : '' }}>
                                                                <small class="text-secondary"><b>Note: The file size should be less than 2MB.</b></small>
                                                                <br>
                                                                <small class="text-secondary"><b>Only .jpg, .jpeg, .png, .webp formats are allowed.</b></small><br>
                                                                @if(isset($entry['image']))
                                                                    <img src="{{ asset('uploads/service/' . $entry['image']) }}" alt="Preview"
                                                                        style="max-width: 100px; margin-top: 10px;">
                                                                    <input type="hidden" name="banner_items[{{ $index }}][old_image]" value="{{ $entry['image'] }}">
                                                                @endif
                                                            </td>
                                                            <td>
                                                                <textarea name="banner_items[{{ $index }}][description]" class="form-control" >{{ $entry['description'] ?? '' }}</textarea>
                                                            </td>
                                                            <td>
                                                                @if($loop->first)
       ...
       @include('components.backend.main-js')
        ...
        <script>
            function previewBannerImage() {
                ...
                // Clear the previous preview if there is any
                previewImage.src = '';
            ...
            function previewImage() {
                ...
                // Clear the previous preview image
                previewImage.src = '';
                ...
                if (file) {
                    ...
                    if (validImageTypes.includes(file.type)) {
                        ...
                        alert('Please upload a valid image file (jpg, jpeg, png, webp).');
                        previewContainer.style.display = 'none';  // Hide preview if invalid file
                    }
                }
            }
